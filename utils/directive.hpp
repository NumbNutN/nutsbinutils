...
public:
    ...
    template<>
    directive<STRING>(std::string str){
    ...
    template<>
    directive<ALIGN>(uint32_t align,uint32_t pos){
    ...
    friend std::ostream& operator<<(std::ostream& out,directive& directive);

};

template <directive_type type>
std::ostream& operator<<(std::ostream& out,directive<type>& directive){
